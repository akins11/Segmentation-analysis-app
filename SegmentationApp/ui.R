ui <- navbarPage(
  ...
  tabPanel(
    ...
    markdown(
      "
      ### **Segmentation Analysis app** `version 0.01`
      
      This application can be used for performing market segmentation analysis 
      using cluster analysis which consist of dividing a market into groups
      of similar characteristics like age, income, personality, purchasing and
      general behavior. you can read more on market segmentation on [Wikipedia](
      ...
      2. Select numerical (more preferred) and character variables which will 
         ...
         all the available variables in the data. Furthermore less variable mean
         less time for the algorithm to compute the analysis.
      ...
      3. Choosing a cluster algorithm and also the number of centers to 
         ...
         preferred number of centers. After all important inputs have been supplied
         the next operation will be to run a cluster analysis.

      4. Creating cluster summaries using the avaliable variables in the data set,
         although this is optional but it help in creating a visual image of
         how the segments should turn out.

      5. Asign preferred segments to the data manually or using an aggregate
