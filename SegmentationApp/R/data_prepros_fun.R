...
get_data_dimension <- function(df) {
  if (is.data.frame(df)) {
    ...
    row_lab <- ifelse(n_row <= 1, "row", "rows") 
    col_lab <- ifelse(n_col <= 1, "column", "columns")
    pref <- ifelse(n_row <= 1, "Data have", "There are")
    
    stringr::str_glue(
      "{pref} **{scales::comma(n_row)}** {row_lab} & **{scales::comma(n_col)}** \\
       {col_lab}"
    )
  }
}
...
drop_data_id <- function(df, id_var = "No Selection") {
  if (id_var != "No Selection") {
    f_tbl <- dplyr::select(df, -tidyselect::all_of(id_var))
    stats::na.omit(f_tbl)
  } else {
    df
  }
